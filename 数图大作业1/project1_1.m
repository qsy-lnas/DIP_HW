...
for i = 0: ceil(M / sblock) - 1
    for j = 0: ceil(N / sblock) - 1
        x0 = ((i * sblock - sblock) < 1) + ((i * sblock - sblock) >= 1) * (i * sblock - sblock); 
        y0 = ((j * sblock - sblock) < 1) + ((j * sblock - sblock) >= 1) * (j * sblock - sblock); 
        x1 = ((i * sblock + (lblock - sblock) / 2 + sblock - 1) > M) * M + ...
            ((i * sblock + (lblock - sblock) / 2 + sblock - 1) <= M) * (i * sblock + (lblock - sblock) / 2 + sblock - 1); 
        y1 = ((j * sblock + (lblock - sblock) / 2 + sblock - 1) > N) * N + ...
            ((j * sblock + (lblock - sblock) / 2 + sblock - 1) <= N) * (j * sblock + (lblock - sblock) / 2 + sblock - 1);         
...
for i = 0:ceil(M / sblock) - 1
    for j = 0:ceil(N / sblock) - 1
        ...
        if block(x1, y1) > th
            Direction(i + 1, j + 1) = atand((y1 - y2) / (x1 - x2)); 
